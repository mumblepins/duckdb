...
static double AffineGapDistance(const string_t &s1, const string_t &s2, double matchWeight = 1,
                                double mismatchWeight = 11, double gapWeight = 10, double spaceWeight = 7,
	...
	if (s1_len == s2_len && s1_str == s2_str && matchWeight <= mismatchWeight && matchWeight <= gapWeight) {
	...
	for (idx_t i = 1; i <= s2_len; i++) {
		...
		for (idx_t j = 1; j <= s1_len; j++) {
			...
			if (j <= s2_len) {
				...
				I = (std::min(I, V_current[j - 1] + gapWeight * abbreviationScale) + spaceWeight * abbreviationScale);
			}
...
static double NormalizedAffineGapDistance(const string_t &s1, const string_t &s2, double matchWeight = 1,
                                          double mismatchWeight = 11, double gapWeight = 10, double spaceWeight = 7,
                                          double abbreviationScale = .125) {
	...
	double distance = AffineGapDistance(s1, s2, matchWeight, mismatchWeight, gapWeight, spaceWeight, abbreviationScale);
	return distance / (len1 + len2);
}

static double AffineGapDistanceScalarFunction(Vector &result, const string_t s1, const string_t s2,
                                              double matchWeight = 1, double mismatchWeight = 11, double gapWeight = 10,
                                              double spaceWeight = 7, double abbreviationScale = .125) {
	return (double)AffineGapDistance(s1, s2, matchWeight, mismatchWeight, gapWeight, spaceWeight, abbreviationScale);
}

static double NormalizedAffineGapDistanceScalarFunction(Vector &result, const string_t s1, const string_t s2,
                                                        double matchWeight = 1, double mismatchWeight = 11,
                                                        double gapWeight = 10, double spaceWeight = 7,
                                                        double abbreviationScale = .125) {
	return (double)NormalizedAffineGapDistance(s1, s2, matchWeight, mismatchWeight, gapWeight, spaceWeight,
	                                           abbreviationScale);
}

static void AffineGapFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	if (args.ColumnCount() == 2) {
		BinaryExecutor::Execute<string_t, string_t, double>(
		    ...
		    [&](string_t s1, string_t s2) { return AffineGapDistanceScalarFunction(result, s1, s2); });
	} else {
		SeptenaryExecutor::Execute<string_t, string_t, double, double, double, double, double, double>(
		    args, result, [&](string_t s1, string_t s2, double mw, double mmw, double gw, double sw, double as) {
			    return AffineGapDistanceScalarFunction(result, s1, s2, mw, mmw, gw, sw, as);
		    });
	}
}

static void NormalizedAffineGapFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	D_ASSERT(args.ColumnCount() == 2 || args.ColumnCount() == 7);
	auto &s1_vec = args.data[0];
	auto &s2_vec = args.data[1];
	if (args.ColumnCount() == 2) {
		BinaryExecutor::Execute<string_t, string_t, double>(
		    ...
		    [&](string_t s1, string_t s2) { return NormalizedAffineGapDistanceScalarFunction(result, s1, s2); });
	} else {
		SeptenaryExecutor::Execute<string_t, string_t, double, double, double, double, double, double>(
		    args, result, [&](string_t s1, string_t s2, double mw, double mmw, double gw, double sw, double as) {
			    return NormalizedAffineGapDistanceScalarFunction(result, s1, s2, mw, mmw, gw, sw, as);
		    });
	}
}
...
static void LoadInternal(DatabaseInstance &instance) {
	...
	affine_gap.AddFunction(ScalarFunction("affine_gap", {LogicalType::VARCHAR, LogicalType::VARCHAR},
	                                      LogicalType::DOUBLE, AffineGapFunction));
	affine_gap.AddFunction(
	    ScalarFunction("affine_gap",
	...
	normalized_affine_gap.AddFunction(ScalarFunction("normalized_affine_gap",
	                                                 {LogicalType::VARCHAR, LogicalType::VARCHAR}, LogicalType::DOUBLE,
	                                                 NormalizedAffineGapFunction));
	normalized_affine_gap.AddFunction(
	    ScalarFunction("normalized_affine_gap",
	                   {LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::DOUBLE, LogicalType::DOUBLE,
	                    LogicalType::DOUBLE, LogicalType::DOUBLE, LogicalType::DOUBLE},
	                   LogicalType::DOUBLE, NormalizedAffineGapFunction));
	CreateScalarFunctionInfo normalized_affine_gap_fun_info(normalized_affine_gap);

	normalized_affine_gap_fun_info.on_conflict = OnCreateConflict::ALTER_ON_CONFLICT;
